...
void Octree::subdivide(const ofMesh & mesh, TreeNode & node, int numLevels, int level) {
	// Checks if level exceeds maximum number of levels to subdivide
    if (level >= numLevels) return;

    // Subdivide box in node into 8 equals side boxes
    vector<Box> eightBoxes;
    subDivideBox8(node.box, eightBoxes);
    
    // Check each box
    for (Box box: eightBoxes) {
        vector<int> pointsRtn;
        int count = getMeshPointsInBox(mesh, node.points, box, pointsRtn);
        
        // Checks if child box has at least one point
        if (count >= 1) {
            ...
            // Checks if child is not leaf node
            // In other words, there is more than 1 point in the child box
            if (count > 1) {
                subdivide(mesh, node.children.back(), numLevels, level);
                
                // Increment level to indicate that it has reached a new level
                level++;
            } else { /* Else, it is a leaf node */
                // Increments number of leaf nodes
                numLeaf++;
            }
        }
    }
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn) {
    ...
    // Checks if node's box intersects with a ray
    if (node.box.intersect(ray, -distance, distance)) {
        
        // Check if the node is a leaf node by checking if it has no
        // child nodes
        if (node.children.empty()) {
            nodeRtn = node;
            intersects = true;
        }
        else { /* Keep checking until it reaches leaf nodes */
...
bool Octree::intersect(const Ray &ray, const TreeNode & node, TreeNode & nodeRtn, int numLevels, int level) {
    ...
    // Checks if level exceeds maximum number of levels to subdivide
    if (level >= numLevels) return intersects;
    
    // Increment level to indicate that it has reached a new level
    level++;
    
    // Checks if node's box intersects with a ray
    if (node.box.intersect(ray, 0, distance)) {
        
        // Check if it arrives at a certain level
        if (level == numLevels) {
            nodeRtn.children.push_back(node);
            intersects = true;
        }
        else { /* Keep checking until it reaches leaf nodes */
            
            for (TreeNode child: node.children) {
                if (intersect(ray, child, nodeRtn, numLevels, level)) {
                    intersects = true;
                    break;
                }
            }
...
bool Octree::intersect(const Box &box, TreeNode & node, vector<Box> & boxListRtn) {
    ...
    // Check if node's box overlaps the box
    if ((intersects = node.box.overlap(box))) {
        
        // Check if it is a leaf node in order to push back the
        // node's box
        if (node.children.empty()) {
            boxListRtn.push_back(node.box);
            
        } else { /* Keep checking for leaf node */
            for (TreeNode child: node.children) {
...
void Octree::draw(TreeNode & node, int numLevels, int level) {
    ...
    // return if level exceeds numLevels
    if (level >= numLevels) return;
    ...
    // Draw each child node in the parent node
    for (TreeNode child: node.children) {
        ofSetColor(colors[level % colors.size()]); // Switches colors and loops back
        draw(child, numLevels, level);
    }
...
void Octree::drawLeafNodes(TreeNode & node) {
    ...
    } else { /* Keep checking for leaf nodes to draw*/
        for (TreeNode child: node.children) {
            ofNoFill();
