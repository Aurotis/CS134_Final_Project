 ...
 //     assert(min < max);
	...
	//
	 bool overlap(const Box &box) {
         ...
         // Checks if target box's minimum parameters is greater than this box's max parameters and
         // also checks if target box's maximum parameters is less than this box's min parameters
         if (max().x() >= box.parameters[0].x() && min().x() <= box.parameters[1].x()
             && max().y() >= box.parameters[0].y() && min().y() <= box.parameters[1].y()
             && max().z() >= box.parameters[0].z() && min().z() <= box.parameters[1].z()) {
             return true;
         }
